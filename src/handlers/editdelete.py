...
class EditDeleteCog(commands.Cog):
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        if not (message := payload.cached_message) and not (message := self.cache.get_cached_message(payload.channel_id, payload.message_id)) or message.author.bot:
            return
        ...
        embed.set_author(name=message.author.display_name + f" ({message.author.name }, {message.author.id})", icon_url=message.author.avatar.url, url=message.author.avatar.url)
    ...
    async def on_raw_message_edit(self, payload: discord.RawMessageUpdateEvent):
        ...
        if after.author.bot:
            return
        before = payload.cached_message or self.cache.get_cached_message(payload.channel_id, payload.message_id)
        ...
        embed.set_author(name=after.author.display_name + f" ({after.author.name }, {after.author.id})", icon_url=after.author.avatar.url, url=after.author.avatar.url)
        if before:
            embed.add_field(name="Before", value=before.content, inline=True)
        else:
            embed.add_field(name="Before", value="N/A", inline=True)
            self.cache.cache_message(payload.channel_id, after)
